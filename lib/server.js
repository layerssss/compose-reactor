...
class Server {
  constructor(options) {
    ...
    if (Fs.existsSync(this._options.stateFile)) {
      var applicationState = JSON.parse(
  ...
  _doActionImportVolume(parameters, session) {
    ...
    var volumeDockerid = `composereactor${branch.id.replace(
      /-/g,
      ""
    )}_${parameters.volumeName.replace(/[^\w]/g, "")}`;
    ...
    session.onMessage = data => {
      ...
      Promise.resolve()
        ...
        .then(() =>
          this._broadcastMessage("success", "Successfully imported volume!")
        )
  ...
  _doActionLogService(parameters) {
    ...
    this._checkoutBranch(branch, workingPath =>
      this._runTerminal(
        ...
        "docker-compose",
        ["logs", "--follow", "--tail=all", parameters.serviceName],
        workingPath,
        {},
        {
          overrideCode: 1
  ...
  _doActionRunService(parameters) {
    ...
    this._checkoutBranch(branch, workingPath =>
      Promise.resolve()
        .then(
          () =>
            _.find(args, arg => arg.op) &&
            Promise.reject(new Error("Cannot contains shell operators."))
        )
        .then(() =>
          this._runTerminal(
            branch,
            ...
            "docker-compose",
            ["run", parameters.serviceName, ...args],
            workingPath,
            {}
          )
        )
    ...
    this._checkoutBranch(branch, workingPath => {
      ...
      .catch(error =>
        this._broadcastError(
          `Error checkout source code, delete anyway...: ${error.message}`
        )
      )
      .then(() => {
        ...
        .then(async data => {
          ...
          for (const service of services) {
            ...
            for (const port of service.ports || []) {
              ...
              if (!protocol.match(/tcp|udp/)) continue;

              const data = await this._runTerminal(
                branch,
                `Getting ${containerPort}/${protocol} of ${service.name}...`,
                "docker-compose",
                ["port", `--protocol=${protocol}`, service.name, containerPort],
                workingPath,
                {}
              ).catch(() => "");
  ...
  _checkoutBranch(branch, callback) {
    ...
    return Promise.resolve()
      ...
      .then(() => {
        ...
        return Promise.resolve().then(() =>
          this._runTerminal(
            ...
            "git",
            ["clone", branch.repo, "--branch", `${branch.branch}`, repoPath],
            branchPath,
            {}
          )
        );
      ...
      .then(() => {
        var workingPath = branch.path
          ? Path.join(branchPath, branch.path)
          : branchPath;
  ...
  _rebuildProxies() {
    ...
    this._branches.forEach(branch => {
      branch.portMappings.forEach(portMapping => {
        ...
        var service = _.find(
          branch.services,
          s => s.name == portMapping.serviceName
        );
        ...
        var port = _.find(
          service.ports,
          p =>
            p.containerPort == portMapping.containerPort &&
            p.protocol == portMapping.protocol
        );
        ...
        var proxy = _.find(
          this._proxies,
          p => p.publicPort == portMapping.publicPort
        );
        ...
        if (proxy.server) {
          ...
          server.on("error", error => {
            this._broadcastError(
              `Cannot initialize ${portMapping.protocol} port ${
                portMapping.containerPort
              } for ${portMapping.serviceName} of ${branch.repo}#${
                branch.branch
              }: ${error.message}`
            );
  ...
  _doActionMapPort(parameters) {
    for (var existingBranch of this._branches) {
      var existingPort = _.find(
        existingBranch.portMappings,
        p =>
          p.publicPort == parameters.publicPort &&
          p.protocol == parameters.protocol
      );

      if (existingPort) {
        return this._broadcastError(
          `${parameters.publicPort}/${
            parameters.protocol
          } has already been mapped to ${existingBranch.repo}#${
            existingBranch.branch
          }/${existingPort.serviceName}/${existingPort.containerPort}`
        );
      }
    }
    ...
    return Promise.resolve()
      .then(() => {
        ...
        this._sendTerminalOutput(
          terminal.id,
          `${workdir} > ${command} ${args.map(a => `"${a}"`).join(" ")}\r\n`
        );
      })
      .then(
        () =>
          new Promise((resolve, reject) => {
            terminal.pty.on("exit", code => {
              ...
              if (code) {
                if (branch.terminal) branch.terminal.code = code;
                this._broadcastStates();
  ...
  handleWebSocket(socket) {
    ...
    socket.on("message", data => {
      ...
      if (!actionFuncion)
        return this._broadcastError(action.name + " doesn't exist.");
