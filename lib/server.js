...
class Server {
  constructor(options) {
    ...
    if (Fs.existsSync(this._options.stateFile)) {
      ...
      for (var branch of this._branches) {
  ...
  _doActionImportVolume(parameters, session) {
    ...
    session.onMessage = (data) => {
      ...
      Promise.resolve()
        ...
        .then(() => this._runTerminal(
          branch,
          `Dumping from volume ${volumeDockerid}...`,
          'docker', [
            'run',
            '--rm',
            '--volume', `${volumeDockerid}:/mnt/volume`,
            ...
            .then((data) => {
              ...
              return Promise.all(services.map(service => {
                  return Promise.all(service.ports.map(port => {
                      ...
                      if (!protocol.match(/tcp|udp/)) return;
  ...
  _runTerminal(branch, title, command, args, workdir, env) {
    var terminal;
